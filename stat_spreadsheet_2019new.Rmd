...
pbp <- read.csv("C:/Users/Joseph Wysocki/OneDrive/Documents/UVA/Football Research/PFF_pbp.csv")
                 ...
                 columns = list(
                   ...
                   AvgPuntReturn = colDef(
                     ...
                     footer = function(values) {
                       htmltools::tags$b(sprintf("%.3f", mean(values, na.rm = TRUE)))
                     },
...
TwoMinuteCombined <- merge(TwoMinuteEffOff, TwoMinuteDriveOff, all = TRUE) %>% merge(TwoMinuteEffDef, all = TRUE) %>% merge(TwoMinuteDriveDef, all = TRUE) %>% merge(dat1, all.x = TRUE) %>% select(-TeamCode, -PrimaryColor, -SecondaryColor) %>% relocate(teamcode, .after = TeamName)
...
FourMinuteCombined <- merge(FourMinuteEffOff, FourMinuteEffDef, all = TRUE) %>% merge(dat1, all.x = TRUE) %>% select(-TeamCode, -PrimaryColor, -SecondaryColor) %>% relocate(teamcode, .after = TeamName)
